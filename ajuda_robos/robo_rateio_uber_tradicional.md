...
Este robô realiza o processamento mensal do Uber Tradicional.
...
9. Seleciona a organização `MRV Engenharia e Participações S.A.`.
...
14. Converte o relatório CSV para Excel.
15. Valida os valores da coluna `Código da despesa` usando a Base de Centro de Custo.
16. Gera a planilha final de validação.
17. Cria a aba `Rateio` com os valores consolidados.
18. Cria um rascunho no Outlook quando identifica viagens duplicadas que precisam de análise.
19. Abre a pasta do resultado ao finalizar.

## Arquivos gerados

O processo gera os seguintes arquivos na pasta do período:

- Nota de Débito em PDF;
...
- a pasta do mês anterior foi criada no caminho correto;
...
- se a coluna `Validação` foi preenchida;
...
Confirme se seu e-mail está cadastrado como Administrador na organização `MRV Engenharia e Participações S.A.`.
...
- se o código foi preenchido corretamente no relatório da Uber;
